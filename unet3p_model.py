...
from resblock_3d import *
from render import only_rotated_density_field
...
class UNet3DMultiView16(nn.Module):
    ...
    def forward(self, imgList):
        ...
        # new_density = F.interpolate(new_density, size=(96, 176, 96), mode="trilinear", align_corners=False)
        newList = []

        # density_ = only_rotated_density_field(density_[0,0,:,:,:],angle)
            # print(density_.shape)
        #     newList.append(density_)
        # density_ = torch.cat(newList,dim=1)
        # print(density_.shape)


        # density_ = torch.cat([img1,img2], dim=1)
        # x00 = self.input(new_density)
        # x00 = self.sigmoid(x00)
        ...
        # print(x41.shape)
        # short cut
        x01 = self.conv_01_1(x01)  # + density_# [f[0]*5,64^3]->[1,64^3]

        # x01 = torch.clamp(x01, min=0.0, max=1.0)
        # x01 = self.sigmoid(x01)
        # x01 = self.relu(x01)



        x11 = self.upscore2(x11)
        x11 = self.conv_11_1(x11)  # + self.down2(density_)# [f[0]*5,32^3]->[1,32^3]
        # x11 = torch.clamp(x11, min=0.0, max=1.0)
        ...
        x21 = self.conv_21_1(x21)  # + self.down4(density_)# [f[0]*5,16^3]->[1,16^3]
        # x21 = torch.clamp(x21, min=0.0, max=1.0)
        # x21 = self.upscore3(x21)
        # x21 = self.sigmoid(x21)
        # x21 = self.relu(x21)


        x31 = self.upscore4(x31)

        x31 = self.conv_31_1(x31)  # + self.down8(density_)# [f[0]*5,8^3]->[1,8^3]
        # x31 = torch.clamp(x31, min=0.0, max=1.0)
        # x31 = self.upscore4(x31)
        # x31 = self.sigmoid(x31)
        # x31 = self.relu(x31)
        ...
        x41 = self.conv_41_1(x41)  # + self.down16(density_)# [f[0]*5,4^3]->[1,4^3]
        # x41 = torch.clamp(x41, min=0.0, max=1.0)
        # x41 = self.upscore5(x41)
        # x41 = self.sigmoid(x41)
        # x41 = self.relu(x41)



        # x01 = self.MinMaxNorm(x01)
        # x11 = self.MinMaxNorm(x11)
        # x21 = self.MinMaxNorm(x21)
        # x31 = self.MinMaxNorm(x31)
        # x41 = self.MinMaxNorm(x41)
        # x01 = F.interpolate(x01, size=(100, 178, 100), mode="trilinear", align_corners=False)
        # x41 = F.interpolate(x41, size=(100, 178, 100), mode="trilinear", align_corners=False)
        # x41 = F.interpolate(x41, size=(100, 178, 100), mode="trilinear", align_corners=False)
        # x41 = F.interpolate(x41, size=(100, 178, 100), mode="trilinear", align_corners=False)
        # x41 = F.interpolate(x41, size=(100, 178, 100), mode="trilinear", align_corners=False)
...
class UNet3D_trainVelocity(nn.Module):
    ...
    def forward(self, density, randomview=0):
        # density0,1: [1,1,1,64^2]
        # density_ = self.up2D_3D(front_alpha)
        # density_ = torch.cat([img1,img2], dim=1)
...
class UNet3pFtModel(nn.Module):
    # Network Architecture is exactly same as UNet+++ and 3D
    def __init__(self,inchannel=1,outchannel=1,fe = [16, 32, 64, 128, 256],modelconfig = None):
        super(UNet3pFtModel, self).__init__()
        # f = [64, 128, 256, 256, 512]
        # f = [16, 32, 32, 64, 64]
        # f = [16, 32, 64, 128, 256]
        # f = [32, 64, 128, 256, 512]
        ...
        self.up4 = nn.Upsample(scale_factor=(1,4,4), mode='trilinear')
        self.up8 = nn.Upsample(scale_factor=(1,8,8), mode='trilinear')
        self.up16 = nn.Upsample(scale_factor=(1,16,16), mode='trilinear')
        self.down2 = nn.MaxPool3d(kernel_size=(1,2,2))
        self.down4 = nn.MaxPool3d(kernel_size=(1,4,4))
        self.down8 = nn.MaxPool3d(kernel_size=(1,8,8))
        self.down16 = nn.MaxPool3d(kernel_size=(1,16,16))
        # input data conv
        # self.input = nn.Conv3d(16, 1, kernel_size=(3,3,3), stride=1, padding=(1,1,1))
        self.conv_input_00 = nn.Conv3d(inchannel, f[0], kernel_size=(3,3,3), stride=1, padding=(1,1,1))
        # 11*1*1 conv for answer
        self.conv_01_1 = nn.Conv3d(f[0]*5, outchannel, kernel_size=(1,1,1), stride=1, padding=(0,0,0))
        self.conv_11_1 = nn.Conv3d(f[0]*5, outchannel, kernel_size=(1,1,1), stride=1, padding=(0,0,0))
        self.conv_21_1 = nn.Conv3d(f[0]*5, outchannel, kernel_size=(1,1,1), stride=1, padding=(0,0,0))
        ...
        self.res_31_0 = Res_Bottle_Block(f[0]*5,f[0]*5,3,1,1)
        # time resblock
        self.res_00_10 = self.Time_Resblocks(1,[f[0],f[1]],3,(1,2,2),1)# 3
        self.res_10_20 = self.Time_Resblocks(1,[f[1],f[2]],3,(1,2,2),1)# 4
        self.res_20_30 = self.Time_Resblocks(1,[f[2],f[3]],3,(1,2,2),1)# 6
        self.res_30_40 = self.Time_Resblocks(1,[f[3],f[4]],3,(1,2,2),1)# 3

        #upsample
        self.upscore5 = nn.Upsample(scale_factor=(1,16,16), mode='nearest')
        self.upscore4 = nn.Upsample(scale_factor=(1,8,8), mode='nearest')
        self.upscore3 = nn.Upsample(scale_factor=(1,4,4), mode='nearest')
        self.upscore2 = nn.Upsample(scale_factor=(1,2,2), mode='nearest')
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, imgList):
        #imgList [b,c,2,64,64]
        # img = imgList.permute(0,2,1,3,4) #[b,3,1,64,64]
        # density_ = img.repeat(1, 1, self.ch, 1, 1).float() #[b,3,64,64,64]
        # new_density = density_.clone()
        # for idx,angle in enumerate(self.angleList):
        #     rho = only_rotated_density_field(density_[0, idx, :, :, :], angle)
        #     new_density[0,idx,:,:,:] = rho
        ...
        # x00 = self.input(new_density)
        # x00 = self.sigmoid(x00)
        x00 = self.conv_input_00(imgList)# [2,64^3]->[f[0],64^3]
        # print(x00.shape)
        x10 = self.res_00_10(x00)# [f[0],32^3]->[f[1],32^3]
        # print(x10.shape)
        x20 = self.res_10_20(x10)# [f[1],16^3]->[f[2],16^3]
